...
public class MT_Table_InstanceReward extends MT_TableBase {
	final String FILE_MD5_CODE = "67b5430339bfc3d9c9933fc5957d3e4c";
    private String fileName = "";
    private HashMap<Integer,MT_Data_InstanceReward> m_dataArray = new HashMap<Integer,MT_Data_InstanceReward>();

    public MT_Table_InstanceReward (String fileName) {
    ...
    public void Initialize() throws Exception {
        ...
        String strClassMD5CodeInstanceReward = TableUtil.ReadString(reader);
        if (!strClassMD5CodeInstanceReward.equals(InstanceReward.MD5))
            throw new Exception("文件" + fileName + "的自定义类[InstanceReward]已被修改");
        ...
        for (Integer i = 0; i < iRow; ++i) {
            MT_Data_InstanceReward pData = MT_Data_InstanceReward.ReadMemory(reader, fileName);
	...
	public MT_Data_InstanceReward GetElement(Integer ID) {
		if (Contains(ID))
			return m_dataArray.get(ID);
        TableUtil.Warning("MT_Data_InstanceReward key is not exist " + ID);
		return null;
	}
	...
	public final HashMap<Integer,MT_Data_InstanceReward> Datas() {
		return m_dataArray;
	}
