...
public class MT_Table_GGamePolicy extends MT_TableBase {
	final String FILE_MD5_CODE = "f4f1e511380b4397852db3e35130a168";
    private String fileName = "";
	private Boolean newFlag = true;
    private HashMap<Integer,MT_Data_GGamePolicy> m_dataArray = new HashMap<Integer,MT_Data_GGamePolicy>();

    public MT_Table_GGamePolicy (String fileName) {
    ...
    public void Initialize() throws Exception {
        ...
        String strClassMD5CodeLong3 = TableUtil.ReadString(reader);
        if (!strClassMD5CodeLong3.equals(Long3.MD5))
            throw new Exception("文件" + fileName + "的自定义类[Long3]已被修改");
        ...
        for (Integer i = 0; i < iRow; ++i) {
            MT_Data_GGamePolicy pData = MT_Data_GGamePolicy.ReadMemory(reader, fileName);
	...
	public MT_Data_GGamePolicy GetElement(Integer ID) {
		if (Contains(ID))
			return m_dataArray.get(ID);
        TableUtil.Warning("MT_Data_GGamePolicy key is not exist " + ID);
		return null;
	}
	...
	public final HashMap<Integer,MT_Data_GGamePolicy> Datas() {
		return m_dataArray;
	}
